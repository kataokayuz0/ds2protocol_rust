...
pub fn main_function(party_number: usize) {
    ...
    let alpha: f64 = 11.0 * f64_party_number * 5.0;
    let kappa_usize = 60;
    let gamma: f64 = 1.1;
    let t: f64 = 12.0;
    let epsilon: f64 = 0.1;
...
impl Polynomial {
    ...
    pub fn mul_ntt(&self, other: &Polynomial) -> Self {
        ...
        // モジュラス Q による剰余を取り、i128へ型変換
        let mod_prod = prod.iter().map(|&x| (x as i128 % self.mod_val)).collect();
