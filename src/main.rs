...
const K: usize = 2;
const L: usize = 1;
const Q: i128 = 7881299347898369;
const PRIMITIVE_ROOT: i128 = 3; //原始根
const N: usize = 2048;
...
pub fn main_function(party_number: usize) {
    ...
    let sn: Vec<Vec<Polynomial>> = (0..party_number)
        .map(|_| sample_from_s_eta(eta, L + K, Q))
        .collect();
    ...
    let alpha: f64 = 11.0 * f64_party_number * 30.0;
    let kappa_usize = 60;
    let gamma: f64 = 1.1;
    let t: f64 = 12.0;
    let epsilon: f64 = 0.1;
    ...
    {
        let reconted_wj = recon_wj(&a_bar, &csn, &sample_yn, derived_challenge, &tn, &Q, K);
...
impl Polynomial {
    ...
    fn new(coeffs: Vec<i128>, mod_val: i128) -> Self {
        if mod_val == 0 {
...
fn ntt(data: &mut Vec<i128>, modulus: i128, invert: bool) {
    ...
    let mut len = 2;
    while len <= n {
        let wlen = mod_pow(PRIMITIVE_ROOT, (modulus - 1) / len as i128, modulus);
        for i in 0..n / len {
            let mut w = 1;
            for j in 0..len / 2 {
                let u = data[i * len + j];
    ...
    if invert {
        let inv_n = mod_pow(n as i128, modulus - 2, modulus);
        for x in data.iter_mut() {
            *x = (*x * inv_n) % modulus; // 修正: すべての要素に inv_n を乗じる
        }
    }
}
...
fn sample_from_s_eta(eta: i128, size: usize, mod_val: i128) -> Vec<Polynomial> {
    ...
    (0..size)
        .map(|_| {
            ...
            Polynomial::new(vec![modded_coeff], mod_val)
        })
        .collect()
}
...
fn calculate_sums_for_poly(csn: &Vec<f64>, zn: &Vec<f64>, s: f64) -> (f64, f64) {
    ...
    let csn_norm = calculate_norm(csn);

    let rohs_rm = (-PI * zn_norm / s.powi(2)).exp();
    let rohcsn_s_rm = (-PI * csn_norm / s.powi(2)).exp();
...
fn recon_wj(
    ...
    // a_bar と csn の乗算を行い、その結果を保存
    let a_bar_csn_result = multiply_polynomial_matrix_vector(a_bar, csn, q);
    // println!("a_bar_csn_result: {:?}", a_bar_csn_result);

    // a_bar と sample_yn の乗算を行い、その結果を保存
    let a_bar_sample_yn_result = multiply_polynomial_matrix_vector(a_bar, sample_yn, q);
    // println!("a_bar_sample_yn_result: {:?}", a_bar_sample_yn_result);

    // a_bar_csn_result と a_bar_sample_yn_result の加算を行い、その結果を保存
    let addition_result: Vec<Vec<Polynomial>> = a_bar_csn_result
        .iter()
        .zip(a_bar_sample_yn_result.iter())
        .map(|(csn_row, sample_row)| {
            csn_row
                .iter()
                .zip(sample_row)
                .map(|(csn_poly, sample_poly)| {
                    let mut sum = csn_poly.clone().add(sample_poly.clone()); // 加算
                                                                             // sum.normalize(); // 加算後に正規化
                    sum
                })
                .collect()
        })
